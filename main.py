...
def video(v:str,response: Response,request: Request,yuki: Union[str] = Cookie(None),proxy: Union[str] = Cookie(None)):
    if not(check_cokie(iPad4610)):
        return redirect("/")
    response.set_cookie(key="iPad4610", value="True",max_age=7*24*60*60)
    videoid = v
    t = get_data(videoid)
    response.set_cookie("iPad4610","True",max_age=60 * 60 * 24 * 7)
...
def channel(channelid:str,response: Response,request: Request,iPad4610: Union[str] = Cookie(None),proxy: Union[str] = Cookie(None)):
    if not(check_cokie(iPad4610)):
        return redirect("/")
    response.set_cookie("iPad4610","True",max_age=60 * 60 * 24 * 7)
